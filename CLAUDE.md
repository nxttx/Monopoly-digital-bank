...
- API error mapping: handlers translate domain exceptions into typed results (`Conflict`, `BadRequest`, …) — each mapping added only when a user test asserts it. Command-log caveats (accepted): replay re-runs domain guards (stricter future rules can make old logs unreplayable — `version` column exists for evolution) and BankCard data re-rolls on every replay.
